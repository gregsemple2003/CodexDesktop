...
Unlocking the Codex harness: how we built the App Server | OpenAI Table of contents
...
events as content streams in (for streaming item types)

- item/completed

when the item finalizes with its terminal payload

This lifecycle lets clients start rendering immediately on

started

, stream incremental updates on
...
At the beginning of the conversation, the client and the server need to establish the

initialize

handshake. The client must send a single

initialize

request before any other method, and the server acknowledges with a response. This gives the server a chance to advertise capabilities and lets both sides agree on protocol versioning, feature flags, and defaults before the real work begins. Here’s an example payload from OpenAI’s VS Code extension:
...
When a client makes a new request, it will first create a thread and then a turn. The server will send back notifications for progress (
...
In the end, the server sends an agent message and then ends the turn with

turn/completed

. The agent message delta events stream pieces of the message back until the message is finalized with
...
We use a “JSON‑RPC lite” variant: it keeps the request/response/notification shape, but omits the

"jsonrpc": "2.0"

header and is framed as JSONL over stdio rather than strict JSON‑RPC 2.0.
...
View all From model to agent: Equipping the Responses API with a computer environment
...
(opens in a new window) (opens in a new window) (opens in a new window) (opens in a new window) (opens in a new window) (opens in a new window) (opens in a new window) OpenAI © 2015–2026 Manage Cookies English United States
